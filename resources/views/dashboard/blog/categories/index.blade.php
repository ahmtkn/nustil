...
@section('content')
    ...
    <section class="max-w-7xl mx-auto"
             x-data="{deleting:null,action:null,url:'{{route('dashboard.blog.categories.delete',['category'=>'CATEGORY_ID'])}}'}">
            ...
            @foreach($categories as $category)
                ...
                @if($category->descendants->count())
                    @foreach($category->descendants as $descendant)
                        @include('dashboard.categories.partials.tree-view',['category'=>$descendant,'isBlog'=>true])
                    @endforeach
                @endif
            @endforeach
            </tbody>
